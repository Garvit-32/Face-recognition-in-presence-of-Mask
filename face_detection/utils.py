...
def add_borders(curr_img, target_shape=(224, 224), fill_type=0):
    ...
    image = cv2.copyMakeBorder(
        curr_img, shift_h // 2, (shift_h + 1) // 2, shift_w // 2, (shift_w + 1) // 2, fill_type)
...
def resize_image(image, target_size, resize_factor=None, is_pad=True, interpolation=3):
    ...
    if resize_factor is None and is_pad:
        resize_factor = min(
            target_size[0] / curr_image_size[0], target_size[1] / curr_image_size[1])
    elif resize_factor is None and not is_pad:
        resize_factor = np.sqrt(
            (target_size[0] * target_size[1]) / (curr_image_size[0] * curr_image_size[1]))

    image = cv2.resize(image, None, None, fx=resize_factor,
